...
opts.pPyramid.pChns.shrink=2; opts.name='F:\CJ\Codes\matlav\DollarsDemoCaltech/model/AcfCaltech+';
...
for i=1:length(imgNms)
    I=imread(imgNms{i});
    tic, bbs=acfDetect(I,detector); toc
%     figure(1); im(I); bbApply('draw',bbs); %显示
    boxedI = bbApply( 'embed', I, bbs);%输出
    savename=regexp(imgNms{i}, '\', 'split');savename=savename{7};%输出
    imwrite(boxedI,sprintf('%s\\%s',outputDir,savename)); %输出
%     pause(1);  %显示
end
end
