function pyramid = mychnsPyramid( I, varargin )
% Compute channel feature pyramid given an input image.
%
% While mychnsCompute() computes channel features at a single scale,
% chnsPyramid() calls mychnsCompute() multiple times on different scale
...
% calls mychnsCompute() with the specified "pChns" on each scale of the image
...
%
% While calling mychnsCompute() on each image scale works, it is unnecessary.
...
% corresponding channel at the nearest true scale (computed w mychnsCompute)
...
% and only power of two scales are actually computed (using mychnsCompute).
...
%   .pChns        - parameters for creating channels (see mychnsCompute.m)
...
%   .info         - [nTypes x 1] struct array (mirrored from mychnsCompute)
...
%
% See also mychnsCompute, chnsScaling, convTri, imPad
%
...
if( ~isfield(p,'complete') || p.complete~=1 || isempty(I) )
  ...
  p=getPrmDflt(varargin,dfs,1); chns=mychnsCompute([],p.pChns);
...
for i=isR
  ...
  chns=mychnsCompute(I1,pChns); info=chns.info;
