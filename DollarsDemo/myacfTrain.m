...
% 'pPyramid'. See mychnsCompute.m and chnsPyramid.m for more details. The
...
%   .info       - info about channels (see mychnsCompute.m)
%
% EXAMPLE
%
% See also acfReadme, acfDetect, acfDemoInria, acfModify, acfTest,
% mychnsCompute, chnsPyramid, adaBoostTrain, bbGt, bbNms, jitterImage
%
...
%     pChns.pGradMag.enabled=0;
 %   t=mychnsCompute(t,opts.pPyramid.pChns); detector.info=t.info;
  ...
  if( stage==0 && length(opts.filters)==2 )
    ...
    X1 = mychnsCompute1( IsOrig1, opts );
  ...
  X0 = mychnsCompute1( Is0, opts ); clear Is0;
...
if( exist(crDir,'dir') && stage==0 )
  ...
  Is=cell(1,n); 
%   parfor i=1:n, Is{i}=feval(fs{i}{:}); end%CJ
  parfor i=1:n, Is{i}=feval(fs{i}{:}); end%CJ
else
...
function chns = mychnsCompute1( Is, opts )
...
parfor i=1:n
  C=mychnsCompute(Is{i},pChns); C=convTri(cat(3,C.data{:}),smooth);
